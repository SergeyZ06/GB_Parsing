...
def func_load_from_pickle(file_name):
    with open(file=file_name, mode='rb') as file:
        func_list_news = pickle.load(file=file)
    return func_list_news


# Функция для записи информации в БД
def func_write_to_mongo(func_list_news):
    ...
    if mongo_client:
        ...
        collection_columns = list(func_list_news[0].keys())

        # Для каждой записи в списке новостей
        for i in range(len(func_list_news)):
            # вставить в коллекцию новую запись
            mongo_db[mongo_collection_name].insert_one({
                '_id':  i,
                collection_columns[0]:  func_list_news[i][collection_columns[0]],
                collection_columns[1]:  func_list_news[i][collection_columns[1]],
                collection_columns[2]:  func_list_news[i][collection_columns[2]],
                collection_columns[3]:  func_list_news[i][collection_columns[3]]
            })
...
func_write_to_mongo(func_list_news=list_news)
