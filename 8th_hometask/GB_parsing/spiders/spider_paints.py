...
class SpiderPaintsSpider(scrapy.Spider):
    ...
    def parse_item(self, response):
        # Сбор названий характеристик товара
        characteristics_titles = response.xpath(r'//dl[@class="def-list"]/div[@class="def-list__group"]/dt/'
                                                r'text()').getall()
        # Сбор значений характеристик товара
        characteristics_values = response.xpath(r'//dl[@class="def-list"]/div[@class="def-list__group"]/dd/'
                                                r'text()').getall()

        # Словарь для характеристик товара
        dict_characteristics = {}
        # Если список названий характеристик не пустой
        # и кол-во названий характеристик равно кол-ву значений характеристик
        if len(characteristics_titles) == len(characteristics_values) and len(characteristics_titles) > 0:
            # то записать в словарь пары название характеристики и значение характеристики
            for i in range(len(characteristics_titles)):
                dict_characteristics[characteristics_titles[i]] = characteristics_values[i]
        ...
        # харакретистики товара
        my_item_loader.add_value(field_name='characteristics', value=dict_characteristics.copy())
